...
public class Schedule {
    ...
    public ArrayList<Job> runJob;
    public ArrayList<Job> sleepJob;

    public ArrayList<Job> doschedule() {
        int doit = 0;
        ...
        for (int i = 0; i < sleepJob.size(); i++) //对每一个 sleepJob做预测
        {
            ...
            if (nowTime >= startTime)//该job可以开始
            {
                ...
                if (doit == 0)//该job还可以延后执行
                {
                    ...
                    while ((doTime + timeSlice + sleepJob.get(i).getLastTime()) < stopTime) //对每一个合理的时间开始JOB
                    {
